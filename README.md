...
[功能特性](#-功能特性) · [快速开始](#-快速开始) · [部署指南](#-部署指南) · [同步更新](#-同步原项目更新)
...
4. 授权 GitHub 并选择 `lofi-radio-web` 仓库
...
> ⚠️ **注意**: Cloudflare Pages 对 Next.js 的部分功能支持有限，建议参考 [Cloudflare Next.js 指南](https://developers.cloudflare.com/pages/framework-guides/nextjs)。
...
项目内置了 `/api/stations` 接口，可以从 GitHub 动态获取原项目最新电台列表：
...
console.log(data.stations);  // 电台列表
console.log(data.lastUpdated); // 最后更新时间
console.log(data.cached);    // 是否来自缓存
```

接口会自动缓存数据 1 小时，减少对 GitHub API 的请求。
