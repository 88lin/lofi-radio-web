...
const NavBar = memo(({ isDark, isPlaying, currentStation, stationColor, onThemeToggle }: {
  ...
  >
    ...
    >
      <Link href="/" aria-label="Lofi Radio 首页" className="flex items-center gap-2 px-2 py-1 hover:opacity-80 transition-opacity">
      ...
      <div className="relative z-10">
        ...
        <section className="py-6 sm:py-10 px-4 sm:px-6">
          <div className="max-w-5xl mx-auto">
            ...
            >
              {/* 用原生 details/summary 而不是 Radix Accordion：
                  Radix 在未展开时不把内容渲染进 DOM，FAQPage 的 JSON-LD 声明了答案、
                  页面上却没有对应文本，非渲染型 AI 抓取器一条答案都拿不到。 */}
              <div className="w-full">
                {homepageFaqs.map((faq, index) => (
                  ...
                  >
                    <summary
                      className={cn(
                        "flex cursor-pointer list-none items-center justify-between gap-4 py-4 sm:py-5 text-sm sm:text-base font-semibold marker:hidden",
                    ...
                    <p className={cn("pb-4 sm:pb-6 text-sm sm:text-[15px] leading-7 sm:leading-8 pr-2 sm:pr-10 max-w-none", isDark ? "text-white/52" : "text-zinc-600")}>
                      {faq.answer}
                    </p>
                  </details>
                ))}
